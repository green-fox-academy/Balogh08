...
It is not necessary to multiply by 100 and make it to int. It depends on your task, preferences.

### What is a ternary operator? How does it work?

It replaces the "if" statement.

Use character '?' and ':' to check if a statement is true and returns the value after char '?' if the statement is true and returns the value after char ':' if the statement is false.
Example:
int myNumber = 1;
int yourNumber = 2;

int higherNumber = myNumber > yourNumber ? myNumber : yourNumber;

Because the statement is false, myNumber is not greater than yourNumber, value of higherNumber = yourNumber;
